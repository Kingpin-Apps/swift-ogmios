...
public struct QueryLedgerStateEpoch {
    ...
    public struct Request: JSONRPCRequest {
        ...
        init(id: JSONRPCId? = nil) {
            self.id = id
            self.method = QueryLedgerStateEpoch.method
            self.jsonrpc = QueryLedgerStateEpoch.jsonrpc
            self.params = nil
        }
    }
    ...
    public struct Response: JSONRPCResponse {
        public let jsonrpc: String
        public let method: String
        public let result: Epoch
        public let id: JSONRPCId?
        
        init(result: Epoch, id: JSONRPCId? = nil) {
            self.result = result
            self.id = id
            self.method = QueryLedgerStateEpoch.method
            self.jsonrpc = QueryLedgerStateEpoch.jsonrpc
    ...
    private func process(data: Data) async throws -> Response {
        ...
        guard let method = responseJSON["method"] as? String,
                method == QueryLedgerStateEpoch.method else {
            throw OgmiosError.invalidMethodError("Incorrect method for \(QueryLedgerStateEpoch.method) response: \(responseJSON["method"] ?? "nil")")
        }
