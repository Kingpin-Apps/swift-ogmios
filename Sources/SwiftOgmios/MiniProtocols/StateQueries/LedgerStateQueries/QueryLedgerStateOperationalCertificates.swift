...
public struct QueryLedgerStateOperationalCertificates {
    private let client: OgmiosClient
    
    private static let method: String = "queryLedgerState/operationalCertificates"
    ...
    public struct Request: JSONRPCRequest {
        ...
        init(id: JSONRPCId? = nil) {
            self.id = id
            self.method = QueryLedgerStateOperationalCertificates.method
            self.jsonrpc = QueryLedgerStateOperationalCertificates.jsonrpc
            self.params = nil
        }
    }
    ...
    public struct Response: JSONRPCResponse {
        public let jsonrpc: String
        public let method: String
        public let result: OperationalCertificates
        public let id: JSONRPCId?
        
        init(result: OperationalCertificates, id: JSONRPCId? = nil) {
            self.result = result
            self.id = id
            self.method = QueryLedgerStateOperationalCertificates.method
            self.jsonrpc = QueryLedgerStateOperationalCertificates.jsonrpc
    ...
    // MARK: - Public Methods
    public func result(id: JSONRPCId? = nil) async throws -> OperationalCertificates {
    ...
    private func process(data: Data) async throws -> Response {
        ...
        guard let method = responseJSON["method"] as? String,
              method == QueryLedgerStateOperationalCertificates.method else {
            throw OgmiosError.invalidMethodError("Incorrect method for \(QueryLedgerStateOperationalCertificates.method) response: \(responseJSON["method"] ?? "nil")")
        }
