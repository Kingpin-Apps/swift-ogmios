...
public struct QueryLedgerStateEraStart {
    private let client: OgmiosClient
    
    private static let method: String = "queryLedgerState/eraStart"
    ...
    public struct Request: JSONRPCRequest {
        ...
        init(id: JSONRPCId? = nil) {
            self.id = id
            self.method = QueryLedgerStateEraStart.method
            self.jsonrpc = QueryLedgerStateEraStart.jsonrpc
            self.params = nil
        }
    }
    ...
    public struct Response: JSONRPCResponse {
        public let jsonrpc: String
        public let method: String
        public let result: Bound
        public let id: JSONRPCId?
        
        init(result: Bound, id: JSONRPCId? = nil) {
            self.result = result
            self.id = id
            self.method = QueryLedgerStateEraStart.method
            self.jsonrpc = QueryLedgerStateEraStart.jsonrpc
    ...
    // MARK: - Public Methods
    public func result(id: JSONRPCId? = nil) async throws -> Bound {
    ...
    private func process(data: Data) async throws -> Response {
        ...
        guard let method = responseJSON["method"] as? String,
                method == QueryLedgerStateEraStart.method else {
            throw OgmiosError.invalidMethodError("Incorrect method for \(QueryLedgerStateEraStart.method) response: \(responseJSON["method"] ?? "nil")")
        }
