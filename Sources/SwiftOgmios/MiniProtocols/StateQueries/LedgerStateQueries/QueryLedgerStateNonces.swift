...
public struct QueryLedgerStateNonces {
    private let client: OgmiosClient
    
    private static let method: String = "queryLedgerState/nonces"
    ...
    public struct Request: JSONRPCRequest {
        ...
        init(id: JSONRPCId? = nil) {
            self.id = id
            self.method = QueryLedgerStateNonces.method
            self.jsonrpc = QueryLedgerStateNonces.jsonrpc
            self.params = nil
        }
    }
    ...
    public struct Response: JSONRPCResponse {
        public let jsonrpc: String
        public let method: String
        public let result: Nonces
        public let id: JSONRPCId?
        
        init(result: Nonces, id: JSONRPCId? = nil) {
            self.result = result
            self.id = id
            self.method = QueryLedgerStateNonces.method
            self.jsonrpc = QueryLedgerStateNonces.jsonrpc
    ...
    // MARK: - Public Methods
    public func result(id: JSONRPCId? = nil) async throws -> Nonces {
    ...
    private func process(data: Data) async throws -> Response {
        ...
        guard let method = responseJSON["method"] as? String,
              method == QueryLedgerStateNonces.method else {
            throw OgmiosError.invalidMethodError("Incorrect method for \(QueryLedgerStateNonces.method) response: \(responseJSON["method"] ?? "nil")")
        }
