...
public struct QueryLedgerStateStakePoolsPerformances {
    private let client: OgmiosClient
    
    private static let method: String = "queryLedgerState/stakePoolsPerformances"
    ...
    public struct Request: JSONRPCRequest {
        ...
        init(id: JSONRPCId? = nil) {
            self.id = id
            self.method = QueryLedgerStateStakePoolsPerformances.method
            self.jsonrpc = QueryLedgerStateStakePoolsPerformances.jsonrpc
            self.params = nil
        }
    }
    ...
    // MARK: - Public Methods
    public func result(id: JSONRPCId? = nil) async throws -> StakePoolsPerformances {
    ...
    private func process(data: Data) async throws -> Response {
        ...
        guard let method = responseJSON["method"] as? String,
              method == QueryLedgerStateStakePoolsPerformances.method else {
            throw OgmiosError.invalidMethodError("Incorrect method for \(QueryLedgerStateStakePoolsPerformances.method) response: \(responseJSON["method"] ?? "nil")")
        }
