...
public struct QueryLedgerStateRewardAccountSummaries {
    private let client: OgmiosClient
    
    private static let method: String = "queryLedgerState/rewardAccountSummaries"
    ...
    public struct Request: JSONRPCRequest {
        ...
        init(id: JSONRPCId? = nil, params: Params) {
            self.id = id
            self.method = QueryLedgerStateRewardAccountSummaries.method
            self.jsonrpc = QueryLedgerStateRewardAccountSummaries.jsonrpc
            self.params = params
        }
    }
    ...
    public struct Response: JSONRPCResponse {
        public let jsonrpc: String
        public let method: String
        public let result: [RewardAccountSummary]
        public let id: JSONRPCId?
        
        init(result: [RewardAccountSummary], id: JSONRPCId? = nil) {
            self.result = result
            self.id = id
            self.method = QueryLedgerStateRewardAccountSummaries.method
            self.jsonrpc = QueryLedgerStateRewardAccountSummaries.jsonrpc
    ...
    // MARK: - Public Methods
    public func result(id: JSONRPCId? = nil, params: Params) async throws -> [RewardAccountSummary] {
    ...
    private func process(data: Data) async throws -> Response {
        ...
        guard let method = responseJSON["method"] as? String,
              method == QueryLedgerStateRewardAccountSummaries.method else {
            throw OgmiosError.invalidMethodError("Incorrect method for \(QueryLedgerStateRewardAccountSummaries.method) response: \(responseJSON["method"] ?? "nil")")
        }
