...
public struct QueryLedgerStateConstitutionalCommittee {
    private let client: OgmiosClient
    
    private static let method: String = "queryLedgerState/constitutionalCommittee"
    ...
    public struct Request: JSONRPCRequest {
        ...
        init(id: JSONRPCId? = nil) {
            self.id = id
            self.method = QueryLedgerStateConstitutionalCommittee.method
            self.jsonrpc = QueryLedgerStateConstitutionalCommittee.jsonrpc
            self.params = nil
        }
    }
    ...
    public struct Result: JSONSerializable {
        ...
        init(members: [ConstitutionalCommitteeMember], quorum: Ratio? = nil) {
    ...
    private func process(data: Data) async throws -> Response {
        ...
        guard let method = responseJSON["method"] as? String,
                method == QueryLedgerStateConstitutionalCommittee.method else {
            throw OgmiosError.invalidMethodError("Incorrect method for \(QueryLedgerStateConstitutionalCommittee.method) response: \(responseJSON["method"] ?? "nil")")
        }
