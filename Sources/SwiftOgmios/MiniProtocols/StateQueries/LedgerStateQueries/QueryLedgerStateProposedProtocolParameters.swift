...
public struct QueryLedgerStateProposedProtocolParameters {
    private let client: OgmiosClient
    private static let method: String = "queryLedgerState/proposedProtocolParameters"
    ...
    public struct Request: JSONRPCRequest {
        ...
        init(id: JSONRPCId? = nil) {
            self.id = id
            self.method = QueryLedgerStateProposedProtocolParameters.method
            self.jsonrpc = QueryLedgerStateProposedProtocolParameters.jsonrpc
            self.params = nil
        }
    }
    ...
    public struct Response: JSONRPCResponse {
        public let jsonrpc: String
        public let method: String
        public let result: [ProposedProtocolParameters]
        public let id: JSONRPCId?
        
        init(result: [ProposedProtocolParameters], id: JSONRPCId? = nil) {
            self.result = result
            self.id = id
            self.method = QueryLedgerStateProposedProtocolParameters.method
            self.jsonrpc = QueryLedgerStateProposedProtocolParameters.jsonrpc
    ...
    // MARK: - Public Methods
    public func result(id: JSONRPCId? = nil) async throws -> [ProposedProtocolParameters] {
    ...
    private func process(data: Data) async throws -> Response {
        ...
        guard let method = responseJSON["method"] as? String,
                method == QueryLedgerStateProposedProtocolParameters.method else {
            throw OgmiosError.invalidMethodError("Incorrect method for \(QueryLedgerStateProposedProtocolParameters.method) response: \(responseJSON["method"] ?? "nil")")
        }
