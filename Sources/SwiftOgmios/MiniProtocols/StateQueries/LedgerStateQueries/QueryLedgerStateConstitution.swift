...
public struct QueryLedgerStateConstitution {
    ...
    public struct Request: JSONRPCRequest {
        ...
        init(id: JSONRPCId? = nil) {
            self.id = id
            self.method = QueryLedgerStateConstitution.method
            self.jsonrpc = QueryLedgerStateConstitution.jsonrpc
            self.params = nil
        }
    }
    ...
    public struct Response: JSONRPCResponse {
        public let jsonrpc: String
        public let method: String
        public let result: Constitution
        public let id: JSONRPCId?
        
        init(result: Constitution, id: JSONRPCId? = nil) {
            self.result = result
            self.id = id
            self.method = QueryLedgerStateConstitution.method
            self.jsonrpc = QueryLedgerStateConstitution.jsonrpc
    ...
    // MARK: - Public Methods
    public func result(id: JSONRPCId? = nil) async throws -> Constitution {
    ...
    private func process(data: Data) async throws -> Response {
        ...
        guard let method = responseJSON["method"] as? String,
                method == QueryLedgerStateConstitution.method else {
            throw OgmiosError.invalidMethodError("Incorrect method for \(QueryLedgerStateConstitution.method) response: \(responseJSON["method"] ?? "nil")")
        }
