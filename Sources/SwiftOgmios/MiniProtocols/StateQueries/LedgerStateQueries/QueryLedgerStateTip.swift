...
/// Query the current tip the ledger is at. Said differently, the slot number and header hash of the last
/// block that has been processed by the ledger.
public struct QueryLedgerStateTip {
    private let client: OgmiosClient
    
    private static let method: String = "queryLedgerState/tip"
    ...
    public struct Request: JSONRPCRequest {
        ...
        init(id: JSONRPCId? = nil) {
            self.id = id
            self.method = QueryLedgerStateTip.method
            self.jsonrpc = QueryLedgerStateTip.jsonrpc
            self.params = nil
        }
    }
    ...
    // MARK: - Public Methods
    public func result(id: JSONRPCId? = nil) async throws -> PointOrOrigin {
    ...
    private func process(data: Data) async throws -> Response {
        ...
        guard let method = responseJSON["method"] as? String,
                method == QueryLedgerStateTip.method else {
            throw OgmiosError.invalidMethodError("Incorrect method for \(QueryLedgerStateTip.method) tip response: \(responseJSON["method"] ?? "nil")")
        }
