...
public struct ReleaseMempool {
    private let client: OgmiosClient
    
    private static let method: String = "releaseMempool"
    ...
    public struct Request: JSONRPCRequest {
        ...
        public init(id: JSONRPCId? = nil) {
            self.id = id
            self.method = ReleaseMempool.method
            self.jsonrpc = ReleaseMempool.jsonrpc
            self.params = nil
        }
    }
    ...
    public struct Result: JSONSerializable {
        ...
        public init(released: String = "mempool") {
    ...
    private func process(data: Data) async throws -> Response {
        ...
        guard let method = responseJSON["method"] as? String,
              method == ReleaseMempool.method else {
            throw OgmiosError.invalidMethodError("Incorrect method for \(ReleaseMempool.method) response: \(responseJSON["method"] ?? "nil")")
        }
